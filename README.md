...
In this GitHub repository, we publish the code search tasks mentioned in our paper, including the descriptions, positive/negative examples, and syntactic notions. We also provide the basic configuration of the synthesis, such as the language schema and the syntactic notion dictionary. The synthesized results under different settings are also included in this repository.
...
### Syntactic Notion Dictionary

All the available syntactic notions are listed in the file `SynthesisConfig/SyntacticNotionDictionary.txt`. We do not provide the details of the syntactic notion signature, which can be easily derived from the API names. For example, the syntactic notion signature of the first attribute of the relation `Method`, i.e., the return value of the API named `getBelongedClass`, includes `belonged` and `class`. Actually, all the matched syntactic notions in the API names form the syntactic notion signature. Such a syntactic notion signature can be derived from the documentation of the APIs, while they almost yield the same result. Due to the commercial use of the Datalog-based program analyzer, we cannot provide the complete documentation and hence, demonstrate the API names to explain how the syntactic notion signatures are generated.
...
A synthesis specification consists of two parts, namely the example program containing positive and negative examples, which is shown in `Main.java`, and the syntactic notions, which are shown in `NL.json`. Also, the description of the code search task is also shown in the file `NL.json`.

In the file `Main.java`, the target constructs are labeled with the comments beginning with `Target`. Here, we use these specific comments to enable the users to achieve the relation partition. In general, we can also design a UI interface for users to drag the code snippets to label the positive and negative examples.


### Synthesized Queries

We list the synthesized queries under three different settings, which leverage the dual quantitative metrics, the syntactic notion coverage, and the structural complexity to prioritize the query candidates, respectively. The results are listed in the files `full-feature.txt`, `coverageonly.txt`, and `sizeonly.txt`, respectively.
